...
router.post('/upload/:roomId',ensureAuthenticated,(req,res)=>{
  if(!req.files)//||req.files.length==0
    ...
    dir="./public/shared/"+req.params.roomId
    if(!fs.existsSync(dir)){
      fs.mkdirSync(dir)
    }
    ipfile.mv(dir+"/"+ipfile.name,(err)=>{
      if(err)
        return res.status(500).send("Could not save the file:"+err)
      res.redirect("//"+req.hostname+':3000/chat/uploadsuccess')
    })
  }
})
