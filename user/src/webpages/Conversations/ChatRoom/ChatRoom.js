...
const ChatRoom = (props) => {
  ...
  const handleAttachment=(event)=>{
    setNewMessage("Shared an attachment: //"+window.location.hostname+":5000/static/shared/"+roomId+"/"+event.target.files[0].name)
  ...
  return (
    ...
    <div style={{border:"1px solid grey",borderRadius:"10px 10px 0 0",background:"rgb(35,35,35)"}}>
      ...
      <Menu id="menu-appbar" anchorEl={anchorEl} anchorOrigin={{vertical: 'top',horizontal: 'right',}} keepMounted transformOrigin={{vertical: 'top', horizontal: 'right',}} open={openprof} onClose={handleClose}>
        <a href={"//"+window.location.hostname+":5000/static/shared/"+roomId} style={{color:"white",textDecoration:"none"}} target="_blank"rel="noopener noreferrer">
          <MenuItem onClick={handleClose}><MediaIcon/>&nbsp;Media</MenuItem>
        </a>
        <Link to="/" style={{color:"white",textDecoration:"none"}}>
          <MenuItem onClick={handleClose}><GroupIcon/>&nbsp;Join another Group</MenuItem>
        </Link>
      </Menu>
      ...
      <div className={classes.margin}>
        <Grid container spacing={1}>
          <Grid item>
            <form method="post" action={"//"+window.location.hostname+":5000/users/upload/"+roomId} encType="multipart/form-data" target="_blank">
            <input accept="file/*" onChange={handleAttachment} className={classes.input} id="contained-button-file" type="file" name="ipfile"/>
            ...
            <button type="submit" id="upload" hidden>Submit</button>
            </form>
          </Grid>
